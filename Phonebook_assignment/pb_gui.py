...
def load_gui(self):
    ...
    I prefer to use grid as it offer the best control
    for placing the widgets.
    """
    ...
    self.lstList1.bind('<<ListboxSelect>>', lambda event: pb_func.onSelect(self, event))
    ...
    self.btn_add = tk.Button(self.master, width=12, height=2, text="Add", command=lambda: pb_func.addToList(self))
    self.btn_add.grid(row=8, column=0, padx=(25, 0), pady=(45, 10), sticky=W)
    self.btn_update = tk.Button(self.master, width=12, height=2, text="Update", command=lambda: pb_func.onUpdate(self))
    self.btn_update.grid(row=8, column=1, padx=(15, 0), pady=(45, 10), sticky=W)
    self.btn_delete = tk.Button(self.master, width=12, height=2, text="Delete", command=lambda: pb_func.onDelete(self))
    self.btn_delete.grid(row=8, column=2, padx=(15, 0), pady=(45, 10), sticky=W)
    self.btn_close = tk.Button(self.master, width=12, height=2, text="Close", command=lambda: pb_func.ask_quit(self))
    self.btn_close.grid(row=8, column=4, columnspan=1, padx=(15, 0), pady=(45, 10), sticky=E)

    pb_func.create_db(self)
    pb_func.onRefresh(self)
