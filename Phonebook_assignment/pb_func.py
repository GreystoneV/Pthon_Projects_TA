    ...
    with conn:
        ...
        if count < 1:
            cur.execute('''INSERT INTO tbl_phonebook (col_fname, col_lname, col_fullname, col_phone, col_email)
                        VALUES (?, ?, ?, ?, ?)''',
                        ('John', 'Doe', 'John Doe', '111=111=111', 'jdoe@email.com'))
    conn.close()
...
def addToList(self):
    ...
    # Force user to provide both names
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0):
        ...
        with conn:
            ...
            if chkName == 0:  # If this is 0 then there is no existence of the fullname and we can add new data
                ...
                self.lstList1.insert(END, var_fullname)  # Update listbox with the fullname
...
def onDelete(self):
    ...
    with conn:
        ...
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel("Delete Confirmation", "All information associated with, ({}) \n will be permenantly deleted from the database. \n\n Proceed with the deletetion request?".format(var_select))
            if confirm:
                conn = sqlite3.connect('db_phonebook.db')
                with conn:
                    cursor = conn.cursor()
                    cursor.execute("""DELETE FROM tbl_phonebook WHERE col_fullname = '{}'""".format(var_select))
                onDeleted(self)  # Call the function to clear all the textboxes and the selected index of listbox
        ...
        with conn:
            ...
            if count == 0 or count2 == 0:  # If proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        ...
                        cursor.execute(
                            ...
                            '{2}'""".format(var_phone, var_email, var_value))
                        onClear(self)
                        conn.commit()
                else:
                ...
                messagebox.showinfo("No changes detected",
                                    ...
                                    "\n\nYour update request has been cancelled.".format(
                                        var_phone, var_email))
            onClear(self)
        conn.close()
    else:
